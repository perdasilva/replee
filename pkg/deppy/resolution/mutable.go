...
func (m *MutableResolutionProblem) ActivateVariable(v deppy.MutableVariable) error {
  if vr, ok := m.variables.GetValue(v.VariableID()); ok {
    if vr.Kind() != v.Kind() {
      return fmt.Errorf("variable %s is not of kind %s", v.VariableID(), v.Kind())
    }
    _, err := vr.Merge(v)
    if err != nil {
      return err
    ...
    m.variables.Put(v.VariableID(), v)
  }
  if activated, err := m.variables.IsActivated(v.VariableID()); err != nil {
    return err
  } else if !activated {
    m.variables.Deactivate(v.VariableID())
  }
  return nil
}
